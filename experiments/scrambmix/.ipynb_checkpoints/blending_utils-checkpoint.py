...
class FloatFrameCutmix(BaseMiniBatchBlending):
    ...
    def do_blending(self, imgs, label, **kwargs):
        ...
        sequence_length = round(self.num_frames * lam)

        adj = torch.randint(0, 2, (1,)) * 2 - 1  # gives -1 or 1 randomly
        adj *= torch.rand(1) * min(lam, 1.0 - lam)
        fade = torch.linspace(lam - adj, lam + adj, steps=sequence_length)
        ...
        label = label_ratio * label + (1 - label_ratio) * label[rand_index, :]
